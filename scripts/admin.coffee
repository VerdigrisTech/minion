...
module.exports = (robot) ->
  ...
  jwt = new googleapis.auth.JWT(
    process.env.HUBOT_GOOGLE_JWT_EMAIL,
    null,
    process.env.HUBOT_GOOGLE_JWT_KEY,
    oauth_scopes,
  ...
  robot.respond /onboard (me )?(.*)$/i, (msg) ->
    robot.emit 'google:authenticate', msg, (err, oauth) ->
      ...
      email_candidates = [
        "#{first_name}@#{process.env.DOMAIN}",
        "#{name.join('')}@#{process.env.DOMAIN}"
      ]

      googleapis.admin('directory_v1').users.insert {
        resource: {}
      }, (err, data) ->
        if err
